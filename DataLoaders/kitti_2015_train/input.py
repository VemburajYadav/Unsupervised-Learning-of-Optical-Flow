...
class KITTI_2015_Train_Dataset(Dataset):

    def __init__(self, datadir=None, multiview_datadir=None, crop_size=None, min_scale=1.0, max_scale=1.0,
        ...
        super(KITTI_2015_Train_Dataset, self).__init__()
        ...
        dataset = KITTI_2015_train(datadir=datadir, mode=mode)

        if self.num_frames > 2:
            self.multiview_path = os.path.join(multiview_datadir, 'image_2')
    ...
    def __len__(self):
        return len(self.data_dict['image_2_pairs'])
    ...
    def __getitem__(self, idx):
        ...
        if self.num_frames > 2:
            img_path = (self.data_dict['image_2_pairs'][idx][0]).split('/')[-1].split('_')[0]
            for i in range(self.num_frames - 2):
                previous_frame = os.path.join(self.multiview_path, img_path + '_0' + str(9-(self.num_frames-3)+i) + '.png')
        ...
        img_1 = np.array(Image.open(self.data_dict['image_2_pairs'][idx][0]), dtype=np.float32)
        img_2 = np.array(Image.open(self.data_dict['image_2_pairs'][idx][1]), dtype=np.float32)
