...
class KITTI_2012_Train_Dataset(Dataset):
    ...
    def __init__(self, datadir=None, multiview_datadir=None, crop_size=None, min_scale=1.0, max_scale=1.0,
        ...
        super(KITTI_2012_Train_Dataset, self).__init__()
        ...
        dataset = KITTI_2012_train(datadir=datadir, mode=mode)

        if self.num_frames > 2:
            self.multiview_path = os.path.join(multiview_datadir, 'image_0')
    ...
    def __len__(self):
        return len(self.data_dict['image_0_pairs'])
    ...
    def __getitem__(self, idx):
        ...
        if self.num_frames > 2:
            img_path = (self.data_dict['image_0_pairs'][idx][0]).split('/')[-1].split('_')[0]
        ...
        img_1 = np.array(Image.open(self.data_dict['image_0_pairs'][idx][0]), dtype=np.float32)
        img_2 = np.array(Image.open(self.data_dict['image_0_pairs'][idx][1]), dtype=np.float32)
